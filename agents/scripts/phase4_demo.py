"""Phase 4 — reasoned escrow lifecycle: two LLM agents genuinely DECIDE, CAW pacts bound them.
  ...
  Evaluator     -> genuinely judges accept/reject (evaluate); its decision picks the on-chain branch

Every state change is a CAW contract_call under a SCOPED pact (contract allowlist + tx-count cap).
Modes (steer the deliverable quality; the accept/reject is still the LLM's call):
  good -> provider does the task well  -> evaluator likely ACCEPTS -> complete -> payout
  bad  -> provider sabotages           -> evaluator likely REJECTS -> reject  -> refund
"""
...
async def main(mode: str) -> None:
    ...
    print(f"\n=== Phase 4 reasoned lifecycle: mode={mode} ===")

    # 1. Client REASONS about funding
    ...
    async with CawWallet(api_url=config.CAW_API_URL, api_key=cc.api_key, wallet_uuid=cc.wallet_id, name="Client") as cw, \
        ...
        client = cw.scoped(await cw.wait_pact_active((await cw.submit_pact(
            intent="Client funds + evaluates escrow jobs", spec=pacts.client_escrow_pact(), name="p4-client")).get("pact_id")))
        provider = pw.scoped(await pw.wait_pact_active((await pw.submit_pact(
            intent="Provider submits deliverables", spec=pacts.provider_pact(), name="p4-provider")).get("pact_id")))
    ...
    print(f"[final] job {job_id} status={proof['final_status']} branch={proof['branch']}")
    print(f"        client {usd(bal0['client'])}->{usd(bal1['client'])}  provider {usd(bal0['provider'])}->{usd(bal1['provider'])}")
    out = Path(__file__).resolve().parent / f"phase4_demo_{mode}_proof.json"
